...
public partial class Library_rptReturn : System.Web.UI.Page
{
    ...
    private void generateReport()
    {
        ...
        if (dataForReport.Rows.Count > 0)
        {
            ...
            stringBuilder.Append("<td style='text-align:center' colspan='5'><span style='font-size:large; font-weight:bold'>Return List Of Books</span>");
            ...
            stringBuilder.Append("<td>Return Date</td>");
            stringBuilder.Append("</tr>");
            int num = 1;
            foreach (DataRow row in (InternalDataCollectionBase)dataForReport.Rows)
            {
                ...
                stringBuilder.Append("<td>" + row["ReturnDate"].ToString() + "</td>");
                stringBuilder.Append("</tr>");
                ++num;
            }
        ...
        if (stringBuilder.ToString() != "")
            Session["rptReturnPrint"] = stringBuilder.ToString();
        else
            Session["rptReturnPrint"] = null;
    }
    ...
    private DataTable getDataForReport()
    {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        if (rdbtnlstUsertype.SelectedValue == "0")
            stringBuilder.Append("select * from Lib_VW_IssueReturnList where ReturnDate is not null and BookCollegeId=" + Session["SchoolId"] + " and CollegeId=" + Session["SchoolId"] + " and MemberType='1'");
        else
            stringBuilder.Append("select * from Lib_VW_IssueReturnListStud where ReturnDate is not null and CollegeId=" + Session["SchoolId"] + " and BookCollegeId=" + Session["SchoolId"]);
        if (txtFrmDate.Text != "")
            stringBuilder.Append(" and ReturnDate >='" + dtpFromdt.SelectedDate + "' and ReturnDate <='" + dtpTodt.SelectedDate + "'");
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        if (Session["rptReturnPrint"] != null)
        {
            try
            {
                ExportToExcel(Session["rptReturnPrint"].ToString());
            }
            catch (Exception ex)
            {
    ...
    private void ExportToExcel(string dataToExport)
    {
        try
        {
            string str = Server.MapPath("Exported_Files/Return_report_" + DateTime.Now.ToString("ddMMyyyyhhmmssfffffff") + ".xls");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Return_report_" + DateTime.Now.ToString("ddMMyyyyhhmmssfffffff") + ".xls");
