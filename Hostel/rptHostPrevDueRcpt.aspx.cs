...
public partial class Hostel_rptHostPrevDueRcpt : System.Web.UI.Page
{
    ...
    private void getdata()
    {
        ...
        if (dataTable1.Rows.Count > 0)
        {
            ...
            if (Request.QueryString["status"] != null)
                dataTable4 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(credit)),TransDesc from dbo.Host_FeeLedger where admnno=" + Convert.ToInt32(Request.QueryString["admnno"]) + " and  Receipt_VrNo=" + RecptNo + " and TransDesc='Previous due' group by TransDesc");
            else
                dataTable4 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(Balance)), TransDesc from dbo.Host_FeeLedger where admnno=" + Convert.ToInt32(Request.QueryString["admnno"]) + " and TransDesc='Previous Balance' group by TransDesc");
            ...
            if (Request.QueryString["status"] != null)
                dataTable6 = clsobj.ExecuteSql("select sum(credit) from dbo.Host_FeeLedger where admnno=" + Convert.ToInt32(Request.QueryString["admnno"]) + " and Receipt_VrNo='" + RecptNo + "' group by AdmnNo");
            else
                dataTable6 = clsobj.ExecuteSql("select sum(balance) from dbo.Host_FeeLedger where admnno=" + Convert.ToInt32(Request.QueryString["admnno"]) + "  and TransDesc='Previous Balance'");
            ...
            ClientScript.RegisterClientScriptBlock(GetType(), "ShowMessage", "alert('No Pending'); window.location='" + "FeeReceiptHostel.aspx" + "';", true);
    }
    ...
    {
        Response.Redirect("HostRcptPrevDue.aspx?admnno=" + Request.QueryString["admnno"].ToString().Trim() + "&sess=" + Request.QueryString["sess"].ToString().Trim() + "&cid=" + Request.QueryString["class"].ToString().Trim() + "&D=p");
    }
    ...
    {
        Response.Redirect("HostelHome.aspx");
    }
