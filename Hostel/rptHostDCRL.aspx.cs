...
public partial class Hostel_rptHostDCRL : System.Web.UI.Page
{
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("rptHostDCRMix.aspx");
        }
        else
        {
            DataSet dataSet2 = common.GetDataSet("HostRptGetALLFeeReceived", hashtable);
    ...
    private DataTable mergertable(DataTable tbstudent)
    {
        ...
        for (int index = 0; index < tbstudent.Rows.Count; ++index)
        {
            ...
            if (hashtable[(str1 + dateTime + str2)] == null)
            {
                hashtable.Add((str1 + dateTime + str2), (str1 + dateTime + str2));
                Common common = new Common();
                DataTable dataTable = new DataTable();
                foreach (DataRow row in (InternalDataCollectionBase)common.GetDataTable("HostRptGetFeeIdNames", new Hashtable()
        {
          {
             "@AdmnNo",
             str1
    ...
    private void GenerateReport(DataSet ds)
    {
        ...
        if (table.Rows.Count > 0)
        {
            ...
            stringBuilder.Append("<td style='width: 80px' align='left'>Admission No(If Any)</td>");
            stringBuilder.Append("<td style='width: 150px' align='left'>Name</td>");
            stringBuilder.Append("<td style='width: 60px' align='left'>Class(If Any)</td>");
            stringBuilder.Append("<td align='left'>Description</td>");
            stringBuilder.Append("<td style='width: 100px' align='right'>Amount</td>");
            stringBuilder.Append("<td style='width: 100px' align='center'>Received By</td>");
        ...
        stringBuilder.Append("<br/><div style='float:right;'><strong >Grand Total &nbsp;:&nbsp; </strong >" + (num1 + num2).ToString() + "</strong>&nbsp;(<strong>In Words :</strong> Rupees&nbsp;" + NumberToText((num1 + num2).ToString()) + " Only)</div>");
        lblReport.Text = stringBuilder.ToString().Trim();
        Session["DCRL"] = stringBuilder.ToString().Trim();
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ScriptManager.RegisterClientScriptBlock((Page)this, GetType(), "ShowMessage", "window.open('rptHostDCRLPrint.aspx');", true);
        }
        catch (Exception ex)
        {
            string message = ex.Message;
    ...
    protected void rbtnConsol_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (!rbtnFeeHead.Checked)
            return;
        Response.Redirect("rptHostDCRMix.aspx?Dt=" + dtpDate.GetDateValue().ToString("dd MMM yyyy"));
    }

    protected void btnCancelled_Click(object sender, EventArgs e)
    {
        Response.Redirect("rptHostCancelledRcpt.aspx?Dt=" + dtpDate.GetDateValue().ToString("dd MMM yyyy"));
    }
