...
public partial class Hostel_rptHostDCRMix : System.Web.UI.Page
{
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        if (rbtnConsol.Checked)
            Response.Redirect("rptHostDCRL.aspx");
        else
            GenerateReport();
    }
    ...
    private void GenerateReport()
    {
        ...
        if (ViewState["TotFee"] != null)
            num = Convert.ToDecimal(ViewState["TotFee"].ToString().Trim());
        if (num != new Decimal(0))
        {
            lblTotal.Text = "<div class='tblheader' align='right'  float='right'><b>Grand Total :&nbsp;</b>" + num + "&nbsp;(In Words : Rupees" + NumberToText(num.ToString()) + ")";
    ...
    private void GetFeeHeadRpt()
    {
        ...
        DataTable dataTable2 = obj.GetDataTable("HostRptGetDCRLMixFee", hashtable);
        if (dataTable2.Rows.Count > 0)
        {
            DataTable dataTable3 = new DataTable();
            DataTable dataTable4 = obj.GetDataTable("HostRptGetDCRMixFeeIdNames");
    ...
    private DataTable mergertable(DataTable tbstudent, int no)
    {
        ...
        for (int index = 0; index < tbstudent.Rows.Count; ++index)
        {
            ...
            if (hashtable1[(str1 + dateTime + str2)] == null)
            {
                ...
                if (no == 1)
                    dataTable = obj.GetDataTable("HostRptGetDCRMixFeeNms", hashtable2);
    ...
    {
        if (rbtnConsol.Checked)
            Response.Redirect("rptHostDCRL.aspx?Dt=" + dtpDate.GetDateValue().ToString("dd MMM yyyy"));
        lblAccHd.Text = "";
        lblFee.Text = "";
        lblTotal.Text = "";
    }
