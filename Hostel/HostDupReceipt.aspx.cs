...
public partial class Hostel_HostDupReceipt : System.Web.UI.Page
{
    ...
    private void fillstudent()
    {
        ...
        if (drpClasses.SelectedIndex != 0)
            dataTableQry = clsDal.GetDataTableQry("select fullname,cs.admnno from PS_ClasswiseStudent cs join ps_studmaster s on s.admnno=cs.admnno inner join dbo.Host_Admission ha on ha.AdmnNo=cs.admnno where cs.classid=" + drpClasses.SelectedValue + " and cs.SessionYear='" + drpSession.SelectedValue.ToString() + "'  order by fullname");
        else
            dataTableQry = clsDal.GetDataTableQry("select  fullname,cs.admnno  from PS_ClasswiseStudent cs join ps_studmaster s on s.admnno=cs.admnno inner join dbo.Host_Admission ha on ha.AdmnNo=cs.admnno where cs.SessionYear='" + drpSession.SelectedValue.ToString() + "'  order by fullname");
    ...
    private void getdata()
    {
        ...
        if (!(Convert.ToDateTime(clsDal.ExecuteScalarQry("select TransDt from dbo.Acts_PaymentReceiptVoucher where InvoiceReceiptNo='" + drpReciept.SelectedValue.ToString().Trim() + "'").Trim().Split(' ')[0]) >= Convert.ToDateTime("4/1/2014")))
            return;
        DataSet dataSet2 = clsDal.GetDataSet("Host_GetDupRcpt", hashtable);
        ...
        stringBuilder1.Append("<td  style='width:35%'><strong>Admn No.:&nbsp;</strong>" + txtadminno.Text.ToString() + "</td><td>&nbsp;</td>");
        stringBuilder1.Append("</tr>");
        ...
        stringBuilder1.Append("</tr>");
        stringBuilder1.Append("<tr>");
        if (table.Rows[0]["TelNoResidence"] != null && table.Rows[0]["TelNoResidence"].ToString().Trim() != "")
            stringBuilder1.Append("<td colspan='4'><strong>Mobile No :</strong>&nbsp;" + table.Rows[0]["TelNoResidence"].ToString() + "&emsp; <strong>Fee Details:</strong>&nbsp;" + dataSet2.Tables[1].Rows[0]["Description"].ToString() + "</td>");
        else
            stringBuilder1.Append("<td colspan='4'><strong>Mobile No :</strong>&nbsp;--&emsp; <strong>Fee Details:</strong>&nbsp;" + dataSet2.Tables[1].Rows[0]["Description"].ToString() + "</td>");
        stringBuilder1.Append("</tr>");
    ...
    private void getConsolidated()
    {
        ...
        if (!(Convert.ToDateTime(clsDal.ExecuteScalarQry("select TransDt from dbo.Acts_PaymentReceiptVoucher where InvoiceReceiptNo='" + drpReciept.SelectedValue.ToString().Trim() + "'").Trim().Split(' ')[0]) >= Convert.ToDateTime("4/1/2014")))
            return;
        DataSet dataSet2 = clsDal.GetDataSet("Host_GetConsolidateDupRcpt", hashtable);
        DataTable table = dataSet2.Tables[0];
        if (table.Rows.Count <= 0)
            return;
        string str = Information();
        ...
        if (dataSet2.Tables[1].Rows[0]["DrawanOnBank"].ToString().Trim() != "")
            stringBuilder2.Append(" Vide </br>&nbsp;Instrument No : <u> " + dataSet2.Tables[1].Rows[0]["ChequeNo"] + "</u>, Dated : <u>" + dataSet2.Tables[1].Rows[0]["ChequeDate"].ToString().Trim() + "</u>, Bank Name : <u>" + dataSet2.Tables[1].Rows[0]["DrawanOnBank"].ToString().Trim() + "</u>");
        stringBuilder2.Append("<tr style='height:30px;'><td colspan='2'></td><td align='right'><b>Received By " + dataSet2.Tables[1].Rows[0]["FullName"].ToString() + "</b></td></tr>");
        stringBuilder2.Append("<tr><td colspan='3' align='center' style='font-size:10px'><u>This is a computer generated receipt</u></td></tr>");
        stringBuilder2.Append("</table>");
        stringBuilder2.Append("</center>");
    ...
    protected void txtadminno_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            btnConsolidat.Enabled = false;
            btnPrint.Enabled = false;
            DataTable dataTableQry = new clsDAL().GetDataTableQry("select SessionYear,ClassID from PS_ClasswiseStudent s inner join dbo.Host_Admission ha on ha.AdmnNo=s.admnno where s.AdmnNo=" + txtadminno.Text.Trim() + " and Detained_Promoted=''");
    ...
    protected void btnConsolidat_Click(object sender, EventArgs e)
    {
        try
        {
            ScriptManager.RegisterClientScriptBlock((Page)this, GetType(), "Message", "window.open('rptHostConsolidatePrint.aspx');", true);
        }
        catch (Exception ex)
        {
