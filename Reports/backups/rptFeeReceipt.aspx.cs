...
public partial class Reports_rptFeeReceipt : System.Web.UI.Page
{
    ...
    private void getdata()
    {
        ...
        DateTime dateTime = Convert.ToDateTime(clsDal.ExecuteScalarQry("SELECT STARTDATE FROM dbo.PS_FeeNormsNew WHERE SessionYr= '" + Request.QueryString["sess"].ToString() + "'"));
        ...
        hashtable.Add("@Due", "C");
        hashtable.Add("@FeeTillDt", Session["FeeTilldate"].ToString());
        ...
        if (dataTable1.Rows.Count > 0)
        {
            ...
            if (Request.QueryString["D"] != null && Request.QueryString["D"].ToString() != "d")
            {
                ...
                string des = Request.QueryString["rno"].ToString();
            ...
            for (int index = 0; index <= dataTable4.Rows.Count - 1; ++index)
            {
                ...
                typeonefee += Convert.ToDouble(dataTable4.Rows[index][0]);
            }
            ...
            Double first = 0.00;
            if (Request.QueryString["status"] != null)
                dataTable6 = clsobj.ExecuteSql("select fs.FeeName as TransDesc,Debit,admnno,Credit,Balance from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID>2 and Receipt_VrNo='" + RecptNo + "'");
            //dataTable6 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(Debit)),fs.FeeName as TransDesc,admnno from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID>2 and Receipt_VrNo='" + RecptNo + "' group by AdmnNo,fs.FeeName order by fs.FeeName");
            else
                dataTable6 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(Balance)),fs.FeeName as TransDesc,admnno,Credit,Balance from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID>2  and TransDate <= getdate() group by AdmnNo,fs.FeeName order by fs.FeeName");
            ...
            //stringBuilder2.Append(CreateIndividualRpt(dataSet2.Tables[1]));
            for (int ind = 0; ind <= dataTable6.Rows.Count - 1; ++ind)
            {
                totalamunt += Convert.ToDouble(dataTable6.Rows[ind][1]);
            }
            if(dataTable6.Rows.Count>0)
            first = Convert.ToDouble(str4) - (totalamunt+ typeonefee - Convert.ToDouble(dataTable6.Rows[0][1]) - Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][4]));
            Double totalamuntak = 0.00;
            for (int index = 0; index <= dataTable6.Rows.Count - 1; ++index)
            {
                ...
                stringBuilder2.Append(".............");
                stringBuilder2.Append("</td><td align='right' style='border-bottom:0;border-top:0;'>");
                if (index == 0)//first fee component
                {
                    stringBuilder2.Append((Convert.ToDecimal(first)).ToString());
                    totalamuntak += Convert.ToDouble(first);
                }
                else if (index == dataTable6.Rows.Count - 1)//end one fee component
                {
                    //if ((Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][1]) - Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][4])) > 0)
                    stringBuilder2.Append((Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][1]) - Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][4])));
                    totalamuntak += (Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][1]) - Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][4]));
                    //else
                    //    stringBuilder2.Append(Convert.ToDouble(dataTable6.Rows[dataTable6.Rows.Count - 1][1]));

                }
                else//middle fee components
                {
                    stringBuilder2.Append((Convert.ToDouble(dataTable6.Rows[index][1])).ToString());
                    totalamuntak += Convert.ToDouble(dataTable6.Rows[index][1]);
                }
                stringBuilder2.Append("</td></tr>");
            ...
            if (Request.QueryString["status"] != null)
                dataTable8 = clsobj.ExecuteSql("select sum(debit) from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and Receipt_VrNo='" + RecptNo + "' group by AdmnNo");
            else
            ...
            if (s1 != "")
            {
                ...
                s1 = "0";
            string s2 = new clsDAL().ExecuteScalarQry("Select CAST(SUM(debit) AS DECIMAL(10,2)) AS BusFee  from PS_FeeLedgerBooks where admnno='" + Convert.ToInt64(Request.QueryString["admnno"]) + "' and Receipt_VrNo='" + RecptNo + "'");
            if (s2 != "")
            {
           ...
           // stringBuilder2.Append(string.Format("{0:F2}", num1)); //closed by akash
            stringBuilder2.Append(string.Format("{0:F2}", totalamuntak+ typeonefee));
            stringBuilder2.Append("</td></tr>");
            double num2 = 0.0;
            double num3 = num1;
            ...
            stringBuilder2.Append("<tr><td colspan='2'></strong>&nbsp;(<strong>In Words :</strong> Rupees&nbsp;" + NumberToText(totalamuntak.ToString().Trim()) + "Only)</td><td>&nbsp;</td></tr>");
            //Common common1 = new Common();
            if (Request.QueryString["D"] != null && Request.QueryString["D"].ToString() != "d")
            {

                if (Convert.ToDouble(str4) <= totalamuntak+ typeonefee)//aka
                {
    ...
    private string CreateIndividualRpt(DataTable dt)
    {
        if (dt.Rows.Count > 0)
        {
    ...
    private void GetDataFullYr()
    {
        ...
        if (dataTable1.Rows.Count > 0)
        {
            ...
            if (Request.QueryString["status"] != null)
                dataTable3 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(Debit)),fs.FeeName as TransDesc,admnno,fs.FeeID from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID<=2 and Receipt_VrNo='" + RecptNo + "' group by AdmnNo,fs.FeeID,fs.FeeName order by fs.FeeID,fs.FeeName");
            else
            ...
            if (Request.QueryString["status"] != null)
                dataTable5 = clsobj.ExecuteSql("select convert(decimal(18,2),sum(Debit)),fs.FeeName as TransDesc,admnno from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID>2 and Receipt_VrNo='" + RecptNo + "' group by AdmnNo,fs.FeeName order by fs.FeeName");
            else
            ...
            if (Request.QueryString["status"] != null)
                dataTable7 = clsobj.ExecuteSql("select sum(debit) from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and Receipt_VrNo='" + RecptNo + "' group by AdmnNo");
            else
    ...
    private void getConsolidated()
    {
        ...
        if (dataTable1.Rows.Count > 0)
        {
            ...
            DataTable dataTable3 = new DataTable();
            if (Request.QueryString["status"] != null)
                clsobj.ExecuteSql("select convert(decimal(18,2),sum(debit)),fs.FeeName as TransDesc,admnno,fs.FeeID from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID<=2 and Receipt_VrNo='" + RecptNo + "' group by AdmnNo,fs.FeeID,fs.FeeName order by fs.FeeID,fs.FeeName");
            else
            ...
            DataTable dataTable4 = new DataTable();
            if (Request.QueryString["status"] != null)
                clsobj.ExecuteSql("select convert(decimal(18,2),sum(debit)),fs.FeeName as TransDesc,admnno from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and PeriodicityID>2 and Receipt_VrNo='" + RecptNo + "' group by AdmnNo,fs.FeeName order by fs.FeeName");
            else
            ...
            DataTable dataTable6;
            if (Request.QueryString["status"] != null)
                dataTable6 = clsobj.ExecuteSql("select sum(debit) from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and Receipt_VrNo='" + RecptNo + "' group by AdmnNo");
            else
            ...
            if (Request.QueryString["status"] != null)
                clsobj.ExecuteSql("select sum(debit) from dbo.PS_FeeLedger fl join dbo.PS_FeeComponents fs on fl.FeeId=fs.FeeID where admnno=" + Convert.ToInt64(Request.QueryString["admnno"]) + " and Receipt_VrNo='" + RecptNo + "' group by AdmnNo");
            else
            ...
            if (Request.QueryString["D"] != null && Request.QueryString["D"].ToString() != "d")
            {
                string str3 = "SELECT isnull(Convert(Decimal(18,2),sum(Amount)),0) AS Fee FROM Acts_PaymentReceiptVoucher WHERE InvoiceReceiptNo= '" + Request.QueryString["rno"].ToString() + "'";
                string str4 = common1.ExecuteScalarQry(str3);
                if(Convert.ToDouble(str4)==num6)
                stringBuilder1.Append("<tr><td colspan='3' style='font-size:12px'>&nbsp;Received Rs " + string.Format("{0:F2}", str4) + ", through " + Request.QueryString["Pmnt"].ToString() + " Payment ");
                else
                    stringBuilder1.Append("<tr><td colspan='3' style='font-size:12px'>&nbsp;Received Rs " + string.Format("{0:F2}", str4) + ", through " + Request.QueryString["Pmnt"].ToString() + " Payment(With Hostel Fee) ");
                if (Request.QueryString["Bn"] != null && Request.QueryString["Bn"].ToString().Trim() != "")
                    stringBuilder1.Append(" Vide </br>&nbsp;Instrument No : <u> " + Request.QueryString["ChkNo"].ToString() + "</u>, Dated : <u>" + Request.QueryString["ChkDt"].ToString() + "</u>, Bank Name : <u>" + Request.QueryString["Bn"].ToString() + "</u>");
                stringBuilder1.Append("</td></tr>");
            }
