...
public partial class Reports_rptDCRMix2 : System.Web.UI.Page
{
    ...
    private void GetFeeHeadRpt()
    {
        ...
        if (drpPmtMode.SelectedIndex > 0)
            hashtable.Add("@PaymentMode", drpPmtMode.SelectedValue.ToString());
        if (rbtnBus.Checked)
            tbstudent = obj.GetDataTable("Ps_Sp_GetDCRLMixFeeBus", hashtable);
        else if (rbtnWoBus.Checked)
            tbstudent = obj.GetDataTable("Ps_Sp_GetDCRLMixFeeWithoutBus", hashtable);
        else if (rbtnAll.Checked)
            tbstudent = obj.GetDataTable("Ps_Sp_GetDCRLMixFee", hashtable);
        if (tbstudent.Rows.Count > 0)
        {
            DataTable tbcomp = new DataTable();
            if (rbtnBus.Checked)
                tbcomp = obj.GetDataTable("Ps_Sp_GetDCRMixFeeIdNamesBus");
            else if (rbtnWoBus.Checked)
                tbcomp = obj.GetDataTable("Ps_Sp_GetDCRMixFeeIdNamesWithoutBus");
            else if (rbtnAll.Checked)
                tbcomp = obj.GetDataTable("Ps_Sp_GetDCRMixFeeIdNames");
            foreach (DataRow row in (InternalDataCollectionBase)tbcomp.Rows)
                tbstudent.Columns.Add(new DataColumn(row["AcctsHeadId"].ToString(), Type.GetType("System.Double")));
            tbstudent.AcceptChanges();
            lblFee.Text = "<div style='background-color:Gray; color:White;'  class='tblheader'  align='left'><b>&nbsp;Fee Heads</b></div>" + CreateReport(mergertable(tbstudent, 1), tbcomp, 1).ToString().Trim();
            btnExpExcel.Enabled = true;
        }
        else
        {
    ...
    private DataTable mergertable(DataTable tbstudent, int no)
    {
        ...
        for (int index = 0; index < tbstudent.Rows.Count; ++index)
        {
            ...
            if (hashtable1[(str1 + dateTime + str2)] == null)
            {
                ...
                {
                    if (rbtnBus.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixFeeNmsBus", hashtable2);
                    else if (rbtnWoBus.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixFeeNmsWithoutBus", hashtable2);
                    else if (rbtnAll.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixFeeNms", hashtable2);
                }
                else if (no == 2)
                {
                    if (rbtnBus.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixAcHdNmsBus", hashtable2);
                    else if (rbtnWoBus.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixAcHdNmsWithoutBus", hashtable2);
                    else if (rbtnAll.Checked)
                        dataTable = obj.GetDataTable("Ps_Sp_GetDCRMixAcHdNms", hashtable2);
                }
                else if (rbtnBus.Checked)
                    dataTable = obj.GetDataSet("Ps_Sp_GetDCRMixAdjustmentBus", hashtable2).Tables[0];
                else if (rbtnWoBus.Checked)
                    dataTable = obj.GetDataSet("Ps_Sp_GetDCRMixAdjustmentWithoutBus", hashtable2).Tables[0];
                else if (rbtnAll.Checked)
                    dataTable = obj.GetDataSet("Ps_Sp_GetDCRMixAdjustment", hashtable2).Tables[0];
    ...
    {
        if (rbtnConsol.Checked)
            Response.Redirect("rptDCRL2.aspx?Dt=" + dtpDate.GetDateValue().ToString("dd MMM yyyy"));
        lblAccHd.Text = "";
        lblFee.Text = "";
        lblTotal.Text = "";
    }

    protected void rbtnFeeHead_CheckedChanged(object sender, EventArgs e)
    {
    }
}
