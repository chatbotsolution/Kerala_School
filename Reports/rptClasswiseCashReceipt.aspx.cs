...
public partial class Reports_rptClasswiseCashReceipt : System.Web.UI.Page
{
    ...
    private void createRemarkReport(DataTable dt)
    {
        ...
        stringBuilder.Append("<td style='width: 200px' align='right'><strong><u>Amount</u></strong></td>");
        stringBuilder.Append("<td style='width: 200px' align='right'><strong><u>Fine Amount</u></strong></td>");
        stringBuilder.Append("</tr>");
        double num1 = 0.0;
        double num2 = 0.0;
        foreach (DataRow row in (InternalDataCollectionBase)dt.Rows)
        {
            ...
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td align='right' style='width: 200px'>");
        ...
        stringBuilder.Append("</tr>");
        stringBuilder.Append("<tr>");
        stringBuilder.Append("<td align='right' colspan='5'>");
        stringBuilder.Append("<strong>Total:" + string.Format("{0:F2}", num1));
        stringBuilder.Append("</strong></td>");
        stringBuilder.Append("<td align='right' colspan='1'>");
        stringBuilder.Append("<strong>Total Fine:" + string.Format("{0:F2}", num2));
        stringBuilder.Append("</strong></td>");
        stringBuilder.Append("</tr>");
        stringBuilder.Append("</table>");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        ...
        stringBuilder.Append("Convert(Decimal(18,2),r.Amount) as Amount, r.Session, c.ClassName , cs.Section,Convert(Decimal(18,2),isnull(fine.Credit,0)) as fine ");
        stringBuilder.Append("from Acts_PaymentReceiptVoucher r inner join PS_StudMaster s on r.PartyId=s.admnno ");
        stringBuilder.Append("inner join PS_ClasswiseStudent cs on r.admnno=cs.admnno and r.class=cs.classid inner join PS_ClassMaster c on cs.classid=c.classid ");
        stringBuilder.Append("left outer join dbo.PS_FineLedger fine on fine.Receipt_VrNo=r.ReceiptNo and fine.Credit > 0 where 1=1 ");
        if (txtFromDate.Text != "")
            stringBuilder.Append(" and r.RecvDate >= '" + str1 + "'");
        else
            stringBuilder.Append(" and r.RecvDate >= '" + str3 + "'");
        if (txtToDate.Text != "")
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("rptClasswiseCashReceiptprint.aspx");
        }
        catch (Exception ex)
        {
            string message = ex.Message;
