...
public partial class Reports_rptClasswiseBankReceipt : System.Web.UI.Page
{
    ...
    private void createRemarkReport(DataTable dt)
    {
        ...
        stringBuilder.Append("<td style='width: 60px' align='right'><strong><u>Amount</u></strong></td>");
        stringBuilder.Append("</tr>");
        double num = 0.0;
        foreach (DataRow row in (InternalDataCollectionBase)dt.Rows)
        {
            ...
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td align='left'>");
            stringBuilder.Append(row["RecDate"].ToString().Trim());
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td align='left'>");
        ...
        stringBuilder.Append("<tr>");
        stringBuilder.Append("<td align='right' colspan='6' style='width: 685px'>");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        ...
        stringBuilder.Append(" select Convert(varchar,r.RecvDate,103)as RecDate , Convert(varchar,r.UserDate,103)as UsersDate , m.AdmnNo, m.FullName, r.Description, Convert(Decimal(18,2),r.Amount) as Amount, r.Session, c.ClassName , cs.Section ");
        stringBuilder.Append(" from  PS_FeeReceiptBank r inner join PS_StudMaster m on m.AdmnNo=r.AdmnNo inner join PS_ClassMaster c on c.ClassID=r.Class   ");
        stringBuilder.Append(" inner join PS_ClasswiseStudent cs on r.AdmnNo=cs.AdmnNo and r.Class=cs.classid where   ");
        if (txtFromDate.Text != "")
        ...
        else
        {
            ...
            stringBuilder.Append(" order by cs.classid, cs.section, RecvDate");
    ...
    protected void btnExpExcel_Click(object sender, EventArgs e)
    {
        try
        {
            if (lblReport.Text.ToString().Trim() != "")
            {
                ...
                stringBuilder.Append("Fee Bank Receipt:-");
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("rptClasswiseBankReceiptprint.aspx");
        }
        catch (Exception ex)
        {
            string message = ex.Message;
