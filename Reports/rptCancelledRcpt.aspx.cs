...
public partial class Reports_rptCancelledRcpt : System.Web.UI.Page
{
    ...
    private void GenerateReport()
    {
        ...
        if (Convert.ToDateTime(dtpFrmDt.GetDateValue().ToString("dd MMM yyyy")) >= Convert.ToDateTime("1 Apr 2014"))
            CreateReportNew(clsDal.GetDataTable("Ps_Sp_GetCancelRcptNew", hashtable));
        else
            CreateReport(clsDal.GetDataTable("Ps_Sp_GetCancelRcpt", hashtable));
    }
    ...
    private void CreateReportNew(DataTable dt)
    {
        if (dt.Rows.Count > 0)
        {
            ...
            stringBuilder.Append("<tr  style='background-color:#eee'>");
    ...
    private void CreateReport(DataTable dt)
    {
        if (dt.Rows.Count > 0)
        {
            ...
            stringBuilder.Append("<td style='width: 30px' align='left'><strong>Sl.No</strong></td>");
            stringBuilder.Append("<td style='width: 100px' align='left'>Cancelled Date</td>");
            stringBuilder.Append("<td style='width: 100px' align='left'>Cancelled Reason</td>");
            ...
            foreach (DataRow row in (InternalDataCollectionBase)dt.Rows)
            {
                ...
                stringBuilder.Append("</td>");
                stringBuilder.Append("<td align='left'>");
                stringBuilder.Append(row["CancelledReason"].ToString().Trim());
                stringBuilder.Append("</td>");
                stringBuilder.Append("<td align='left'>");
    ...
    {
        Response.Redirect("rptCancelledRcptPrint.aspx");
    }
    ...
    private void ExportToExcel(string dataToExport)
    {
        try
        {
            string str = Server.MapPath("Exported_Files/CancelledReceipt" + DateTime.Now.ToString("dd-MM-yyyy hh-mm-ss") + ".xls");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=CancelledReceipt" + DateTime.Now.ToString("dd-MM-yyyy hh-mm-ss") + ".xls");
