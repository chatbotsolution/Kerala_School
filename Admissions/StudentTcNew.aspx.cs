...
public partial class Admissions_StudentTcNew : System.Web.UI.Page
{
    ...
    private void PrintForm()
    {
        ...
        stringBuilder.Append("<tr><td colspan='4' style='padding-left:16px;font-size:17px;'>In Words :<b style='font-family:Times New Roman; font-Size:15px;'><span class='UlSpan'>" + Admissions_StudentTcNew.NumberToDay(dtpDOB.GetDateValue().Day) + " " + dtpDOB.GetDateValue().ToString("MMMM") + " " + Admissions_StudentTcNew.NumberToWords(dtpDOB.GetDateValue().Year) + "</span></b></td></tr>");
        ...
        //stringBuilder.Append("<tr><td colspan='4' style='font-size:17px;'>11. Subjects Studied:1. ENGLISH 2.MATHEMATICS 3.SCIENCE 4.SOCIAL SCIENCE 5.HINDI 6.COMPUTER SCIENCE </span><br>");
        if (drpStream.SelectedValue == "4")
            stringBuilder.Append("<tr><td colspan='4' style='font-size:17px;'>11. Subjects Studied:"+ txtSubStudies.Text.ToString()+" 4."+drpOptSub1.SelectedValue.ToString()+" 5."+drpOptSub2.SelectedValue.ToString()+ " 6."+drpOptSub3.SelectedValue.ToString()+"</span><br>");
        else
            stringBuilder.Append("<tr><td colspan='4' style='font-size:17px;'>11. Subjects Studied:"+ txtSubStudies.Text.ToString()+ "</span><br>");
        ...
        stringBuilder.Append("<tr><td colspan='4' style='padding-left:16px;font-size:17px;'>In Words : <i><span class='UlSpan'>" + Admissions_StudentTcNew.NumberToDay(dtpDOB.GetDateValue().Day) + " " + dtpDOB.GetDateValue().ToString("MMMM") + " " + Admissions_StudentTcNew.NumberToWords(dtpDOB.GetDateValue().Year) + "</span></i></td></tr>");
        //stringBuilder.Append("<tr><td colspan='2' style='font-size:17px;'> &nbsp;&nbsp;&nbsp;&nbsp;Class Of First Admission: <i><span class='UlSpan'>" + hifStudfstClass.Value.ToString() + "</span></i></td></tr>");
        //stringBuilder.Append("<tr><td colspan='2' style='font-size:17px;'>8. Date Of Leaving : <i><span class='UlSpan'>" + txtTcDt.Text.Trim() + "</span></i></td></tr>");
    ...
    protected void drpClass_SelectedIndexChanged(object sender, EventArgs e)
    {
        FillStream();
       // FillSection();
       // drpStream_SelectedIndexChanged(new object(), new EventArgs ());
    ...
    protected void drpSection_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        //hashtable.Add("@Stream", drpStream.SelectedValue.ToString().Trim());
    ...
    {
      {
        ...
        if (dataTable2.Rows.Count > 0)
        {
            ...
            if (dataTable2.Rows[0]["IsueDtCerft"].ToString() != "" && dataTable2.Rows[0]["IsueDtCerft"].ToString() != null)
            {
                //puja code start
                ...
                drpReasonForleave.SelectedValue = dataTable2.Rows[0]["ResLeavingScl"].ToString();
                txtApplReqForCerft.Text = Convert.ToDateTime(dataTable2.Rows[0]["ApplDtReqFrCerft"]).ToString("dd/MM/yyyy");
                txtIsueDtCerft.Text = Convert.ToDateTime(dataTable2.Rows[0]["IsueDtCerft"]).ToString("dd/MM/yyyy");
                ...
                if (drpStream.SelectedValue == "4")
                {
                    ...
                    drpOptSub1.SelectedValue = dataTable2.Rows[0]["SciOPt1"].ToString();
                    drpOptSub2.SelectedValue = dataTable2.Rows[0]["SciOPt2"].ToString();
                    drpOptSub3.SelectedValue = dataTable2.Rows[0]["SciOPt3"].ToString();
                }
            }
            //puja code End
            if (drpStream.SelectedValue == "3")
                txtSubStudies.Text = "1. ENGLISH CORE 2.BUSINESS STUDIES 3.ACCOUNTANCY 4.ECONOMICS  5.PHYSICAL EDUCATION 6.COMPUTER SCIENCE";
            else if (drpStream.SelectedValue == "4")
                txtSubStudies.Text = "1. ENGLISH CORE 2.PHYSICS 3.CHEMISTRY";
    ...
    private void StudExtraDataDetailsEntry()
    {
        if (txtSlno.Text != "")
        {
            ...
            if (drpStream.SelectedValue == "4")
            {
                hashtable.Add("@SciOPt1", drpOptSub1.SelectedValue);
                hashtable.Add("@SciOPt2", drpOptSub2.SelectedValue);
                hashtable.Add("@SciOPt3", drpOptSub3.SelectedValue);
            }
    ...
    public static string NumberToWords(int number)
    {
        if (number == 0)
            return "zero";
        if (number < 0)
            return "minus " + Admissions_StudentTcNew.NumberToWords(Math.Abs(number));
        string str = "";
        if (number / 1000000 > 0)
        {
            str = str + Admissions_StudentTcNew.NumberToWords(number / 1000000) + " million ";
            number %= 1000000;
        }
        if (number / 1000 > 0)
        {
            str = str + Admissions_StudentTcNew.NumberToWords(number / 1000) + " thousand ";
            number %= 1000;
        }
        if (number / 100 > 0)
        {
            str = str + Admissions_StudentTcNew.NumberToWords(number / 100) + " hundred ";
            number %= 100;
        }
        if (number > 0)
        {
...

    {
        ...
        if (drpStream.SelectedValue == "4")
        {
            drpOptSub1.Visible = true;
            drpOptSub2.Visible = true;
            drpOptSub3.Visible = true;
        }
        else  
        {
            drpOptSub1.Visible = false;
            drpOptSub2.Visible = false;
            drpOptSub3.Visible = false;
        }
    }

    private void FillStream()
    {
        drpStream.DataSource = new clsDAL().GetDataTableQry("select [StreamID],[Description] from dbo.[PS_StreamMaster] where  [StreamID] > 2");
