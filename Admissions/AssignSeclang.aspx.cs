...
public partial class Admissions_AssignSeclang : System.Web.UI.Page
{
   ...
   public static int grdrowcount;
    ...
    private void FillGrid()
    {
        ...
        Admissions_AssignSeclang.grdrowcount = grdstudents.Rows.Count;
        if (dataTable.Rows.Count > 0)
        {
           ...
           // BingGridSection();
            ...
            {
                ((DropDownList)grdstudents.Rows[i].FindControl("drpSecLang")).SelectedValue = dataTable.Rows[i]["SecondLang"].ToString();
    ...
    //    }
    //}
    protected void btnUpdtSecLang_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in grdstudents.Rows)
        {
            ...
            DropDownList control2 = (DropDownList)row.FindControl("drpSecLang");
            if (control1.Checked)
            {
                HiddenField control3 = (HiddenField)row.FindControl("hfAdNo");


                hashtable.Add("@SecLang", control2.SelectedValue.ToString());
                ...
                clsDal.GetDataTable("ps_sp_UpdateSeclang", hashtable);
                lblMsg.Text = "Second Language alloted successfully !";
