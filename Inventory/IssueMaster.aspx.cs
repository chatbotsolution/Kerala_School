...
public partial class Inventory_IssueMaster : System.Web.UI.Page
{
    ...
    private void InsertDetails()
    {
        try
        {
            ...
            if (Request.QueryString["IId"] != null)
                ht.Add("IssueId", int.Parse(Request.QueryString["IId"].ToString()));
            ht.Add("IssueDate", dtpdt.GetDateValue());
            ht.Add("IssuedBy", Session["User"]);
            ht.Add("IssuedFromLocId", drpIssuedFrom.SelectedValue);
            if (txtIssuedto.Text.Trim() != "")
                ht.Add("IssuedTo", txtIssuedto.Text.Trim());
            if (drpIssuedtoLoc.SelectedIndex > 0)
                ht.Add("IssuedToLocId", drpIssuedtoLoc.SelectedValue);
            ht.Add("@UserID", Convert.ToInt32(Session["User_Id"]));
            ht.Add("@SchoolId", Convert.ToInt32(Session["SchoolId"]));
            dt = obj.GetDataTable("SI_InsUpdtIssue", ht);
            int int32 = Convert.ToInt32(dt.Rows[0][0].ToString());
            foreach (DataRow row in (InternalDataCollectionBase)dataTable.Rows)
            {
                ht.Clear();
                ht.Add("ItemCode", row["ItemCode"].ToString());
                ht.Add("Qty", row["Qty"].ToString());
                ht.Add("IssueId", int32);
                ht.Add("FromLocId", drpIssuedFrom.SelectedValue.ToString().Trim());
                ht.Add("ToLocId", drpIssuedtoLoc.SelectedValue.ToString().Trim());
                ht.Add("UserID", Convert.ToInt32(Session["User_Id"]));
                ht.Add("SchoolId", Convert.ToInt32(Session["SchoolId"]));
                dt = obj.GetDataTable("SI_InsUpdtIssueDtl", ht);
            }
            if (dt.Rows.Count > 0)
            {
