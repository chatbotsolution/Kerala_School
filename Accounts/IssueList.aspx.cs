...
public partial class Accounts_IssueList : System.Web.UI.Page
{
    ...
    private void bindGrid()
    {
        ...
        if (txtFrmDt.Text != "")
            hashtable.Add("@FromDate", PopCalendar1.GetDateValue());
        if (txtTo.Text != "")
            hashtable.Add("@Todate", PopCalendar2.GetDateValue());
        if (Session["User"].ToString() != "admin")
        {
            hashtable.Add("@SchoolId", Convert.ToInt32(Session["SchoolId"]));
            hashtable.Add("@UserId", int.Parse(Session["User_Id"].ToString()));
        }
        DataTable dataTable2 = obj.GetDataTable("SI_GetAllIssueConsume", hashtable);
    ...
    private void DeleteIssue(string Id)
    {
        ...
        hashtable.Add("@IssueId", Id);
        if (clsDal.GetDataTable("SI_DelIssueConsume", hashtable).Rows.Count > 0)
        {
            lblMsg.Text = "ERROR!";
            lblMsg.ForeColor = Color.Red;
        }
