...
public partial class Accounts_rptActsGroup : System.Web.UI.Page
{
    ...
    private void FillAccountGroupsList()
    {
        ...
        if (dataTable.Rows.Count <= 0)
            return;
        drpParrent.Items.Clear();
        drpParrent.Items.Add(new ListItem("-Parent-", "0"));
        foreach (DataRow row in (InternalDataCollectionBase)dataTable.Rows)
            drpParrent.Items.Add(new ListItem(row["Ag_Name"].ToString(), row["Ag_Code"].ToString()));
    }
    ...
    private void GetReportTable()
    {
        ...
        DataTable dataTable2 = clsDal.GetDataTable("ACTS_GetAccountGroupsRpt", new Hashtable()
    {
      {
         "@AG_Parent",
         drpParrent.SelectedValue.ToString().Trim()
      }
    });
    ...
    private void createDefaultersReport(DataTable dt)
    {
        ...
        stringBuilder.Append("<td colspan='2' style='border-left:0px;' align='left' class='gridtext'><font color='Black'><b>Account Groups Name</b></font>");
        stringBuilder.Append("</td>");
        stringBuilder.Append("</tr>");
        ...
        stringBuilder.Append("<td align='left'class='gridtext'><strong>Account Group Name</strong></td>");
        stringBuilder.Append("<td align='left'class='gridtext'><strong>Parent Category</strong></td>");
        ...
        foreach (DataRow row in (InternalDataCollectionBase)dt.Rows)
        {
            ...
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td align='left' class='gridtext'>");
            stringBuilder.Append(row["Parent"].ToString().Trim());
            stringBuilder.Append("</td>");
            stringBuilder.Append("<td align='left' class='gridtext'>");
            stringBuilder.Append(row["AG_Type"].ToString().Trim());
            stringBuilder.Append("</td>");
            stringBuilder.Append("</tr>");
            ++num;
        }
        ...
        Session["AccGroups"] = stringBuilder.ToString().Trim();
    }
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ScriptManager.RegisterClientScriptBlock((Page)this, GetType(), "Message", "window.open('rptActsGroupPrint.aspx');", true);
        }
        catch (Exception ex)
        {
    ...
    protected void btnExpExcel_Click(object sender, EventArgs e)
    {
        try
        {
            if (lblReport.Text.ToString().Trim() != "")
            {
                ...
                stringBuilder.Append("Account Groups Name :-");
                stringBuilder.Append("</b></td></tr>");
                stringBuilder.Append("<tr><td style='height: 15px'></td></tr>");
                stringBuilder.Append("</table>");
                ExportToExcel((stringBuilder.ToString().Trim() + Session["AccGroups"].ToString()).ToString().Trim());
            }
            else
