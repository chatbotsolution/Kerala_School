...
public partial class Accounts_rptSundryCreditors : System.Web.UI.Page
{
    ...
    protected void btnView_Click(object sender, EventArgs e)
    {
        ...
        DataTable dataTableQry1 = obj.GetDataTableQry("select FY,StartDate from ACTS_FinancialYear where StartDate <='" + PopCalendar1.GetDateValue().ToString("MM/dd/yyyy") + "' and EndDate >='" + PopCalendar1.GetDateValue().ToString("MM/dd/yyyy") + "' ");
        ...
        else
        {
            ...
            DataTable dataTableQry2 = obj.GetDataTableQry("select AG_Code from ACTS_AccountGroups where AG_Code=11");
            if (dataTableQry2.Rows.Count > 0)
            {
                DataTable dataTableQry3 = obj.GetDataTableQry("select PartyName from ACTS_PartyMaster where AcctsHeadId='" + dataTableQry2.Rows[0]["AG_Code"].ToString().Trim() + "' order by PartyName");
                for (int index1 = 0; index1 < dataTableQry3.Rows.Count; ++index1)
                {
                    DataTable dataTableQry4 = obj.GetDataTableQry("select AcctsHeadId from ACTS_AccountHeads where AcctsHead='" + dataTableQry3.Rows[index1]["PartyName"].ToString() + "' ");
                ...
                stringBuilder.Append("<tr><td colspan='4' align='center' class='pageSectionLabel' style= 'border-bottom:dotted 1px black;'><b>List of Sundry Creditors as on " + txtstartdate.Text + "</b></td></tr>");
                stringBuilder.Append("<tr><td colspan='4' height='2px'>&nbsp;</td></tr>");
                stringBuilder.Append("<tr><td colspan='4' height='2px'></td></tr>");
                stringBuilder.Append("<td width='5%'><b>Sno.</b></td>");
                stringBuilder.Append("<td width='30%'><b>Creditors Name</b></td>");
