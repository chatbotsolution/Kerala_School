...
public partial class Accounts_ItemSaleReturn : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                ViewState["SaleInvNo"] = Request.QueryString["InvNo"].ToString();
                lblMsg3.Text = "Sale Inv. Amt on this bill : " + DAL.ExecuteScalarQry("select NetPayableAmt from dbo.ACTS_SaleDoc where invno=" + Request.QueryString["InvNo"].ToString()).ToString();
            }
            else
            {
                ViewState["SaleInvNo"] = null;
                lblMsg3.Text = "";
            }
            ...
            ViewState["Status"] = "";
            Accounts_ItemSaleReturn.edittemp = 0;
            txttotalamt.Text = "";
    ...
    private string SaveData()
    {
        ...
        try
        {
            ...
            hashtable.Add("@InvDate", dtpSaleDate.GetDateValue());
            if (drpSaleList.SelectedIndex == 0)
                hashtable.Add("@AdmnNo", long.Parse(drpstudent.SelectedValue.ToString()));
            ...
            DAL = new clsDAL();
            string str1 = DAL.ExecuteScalarQry("select AcctsHeadId from dbo.PS_AdditionalFeeMaster where Ad_Id=8");
            hashtable.Add("@AcHead", int.Parse(str1.Trim()));
            ...
            if (drpclass.SelectedIndex != 0)
                hashtable.Add("@Class", Convert.ToInt32(drpclass.SelectedValue.ToString()));
            else
                hashtable.Add("@Class", 0);
            if (ViewState["SaleInvNo"] != null)
                hashtable.Add("@SaleInvno", ViewState["SaleInvNo"].ToString());
            Sale sale = new Sale();
            DataTable dataTable1 = (DataTable)ViewState["SalesDetails"];
            long num = 0;
            ...
            try
            {
                SqlCommand sqlCommand = new SqlCommand("dbo.ACTS_InsUpdtItemSaleRet", sqlConnection, transaction);
                ...
                if (str2 != "")
                {
                    ...
                    int32 = (long)Convert.ToInt32(strArray[0].ToString());
                    Rcptno = strArray[1].ToString();
                    if (dataTable1.Rows[0]["ItemCode"].ToString().Trim().Equals(string.Empty))
                        dataTable1.Rows.RemoveAt(0);
                    foreach (DataRow row in (InternalDataCollectionBase)dataTable1.Rows)
                    {
                        if (Convert.ToDecimal(row["Amount"].ToString()) > new Decimal(0))
                        {
                            sqlCommand = new SqlCommand("dbo.ACTS_InsUpdtItemSaleRetDetails", sqlConnection, transaction);
                            sqlCommand.CommandType = CommandType.StoredProcedure;
                            sqlCommand.Parameters.AddWithValue("@InvId", int32);
                            ...
                            sqlCommand.Parameters.AddWithValue("@DiscRate", 0);
                    ...
                    if (dataTable2.Rows.Count > 0)
                        insertBookledger(dataTable2, sqlConnection, transaction);
            ...
            ViewState["InvNo"] = int32;
            empty.Trim().Equals("Data saved successfully !");
        }
        catch (Exception ex)
        {
        }
    ...
    private void insertBookledger(DataTable tb, SqlConnection Conn, SqlTransaction transaction)
    {
        try
        {
            ...
            Decimal.TryParse(txttotalamt.Text, out result1);
            string msg = "";
    ...
    private void InsertBookinLedger(DataRow dr, Decimal CrAmt, SqlConnection Conn, SqlTransaction transaction, out string msg)
    {
        try
        {
            ...
            SqlParameter sqlParameter2 = new SqlParameter("@TransDate", dtpSaleDate.GetDateValue().ToString("dd MMM yyyy"));
            ...
            SqlParameter sqlParameter13 = new SqlParameter("@PayMode", "Cash");
    ...
    protected void btnSubmitPrint_Click(object sender, EventArgs e)
    {
        ...
        {
            clearcontrol();
            ScriptManager.RegisterClientScriptBlock((Control)btnSubmitPrint, btnSubmitPrint.GetType(), "Message", "alert('" + str + "');", true);
            ScriptManager.RegisterStartupScript((Page)HttpContext.Current.Handler, typeof(Page), "Script1", "<script language='javascript' ID='script1'>window.open('ItemSaleMiniInv.aspx?InvNo=" + HttpUtility.UrlEncode(ViewState["SaleInvNo"].ToString()) + "','new window', 'top=40, left=100, width=1100, height=600, dependant=no, location=0, alwaysRaised=no, menubar=no, resizeable=no, scrollbars=n, toolbar=no, status=no, center=yes')</script>", false);
        }
        else
            litFooterMsg.Text = litHeaderMsg.Text = "<div style='padding:5px;font-weight:bold;color:white;background-color:red;border:solid 2px black;'>" + str + "</div>";
    }
    ...
    {
        Response.Redirect("ItemSaleReturnList.aspx");
    }
    ...
    private void CalculateBillAmt()
    {
        ...
        foreach (GridViewRow row in gvItemsForSale.Rows)
        {
            ...
            if (Convert.ToInt32(((Label)row.FindControl("lblAvlQty")).Text.Trim()) < 0)
                control2.Text = "0";
            if (control1.Checked)
                ++num1;
        }
