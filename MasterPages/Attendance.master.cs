...
public partial class MasterPages_Attendance : System.Web.UI.MasterPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (this.Page.IsPostBack)
            return;
        if (this.Session["user"] != null &&Session["SSVMID"] != null)
        {
    ...
    private void CheckPermissions()
    {
        try
        {
            ...
            if (this.Session["permission"] == null)
                return;
            foreach (DataRow row in (InternalDataCollectionBase)(this.Session["permission"] as DataTable).Rows)
            {
                string lower = row["PageID"].ToString().ToLower();
                bool boolean = Convert.ToBoolean(row["status"]);
                if (lower == str.ToLower() && !boolean)
                {
                   Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "ShowMessage", "alert('You are not permitted. Contact your administrator'); window.location='" + "../Administrations/Home.aspx" + "';", true);
                    break;
                }
            }
        }
