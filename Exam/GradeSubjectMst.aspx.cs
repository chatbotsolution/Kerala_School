...
public partial class Exam_GradeSubjectMst : System.Web.UI.Page
{
           ...
           if(drpSelectClass.SelectedItem.Text == "XI" || drpSelectClass.SelectedItem.Text == "XII")
            {
                ...
                DataTable dataTable1 = obj.GetDataTable("GetGradeSubjectList2", new Hashtable()
                  {
                    {
                       "@ClassId",
            ...
            {
                grdSubject.Visible = true;
                DataTable dataTable = obj.GetDataTable("GetGradeSubjectList", new Hashtable()
                                          {
                                            {
                                               "@ClassId",
                                               drpSelectClass.SelectedValue
            ...
            foreach (ListItem listItem in chkListClass.Items)
            {
                if (listItem.Selected)
                {
                    if (ChkListStream.SelectedIndex > -1)
                    {
                        foreach (ListItem listitem2 in ChkListStream.Items)
                        {
                            if (listitem2.Selected)
                            {
                                ...
                                if (obj.ExecuteScalar("InsUpdtGradeSubjectMaster", hashtable1).Trim().ToUpper() == "S")
                                    ++num2;
                                else
                                    ++num1;
                            }
                    ...
                    else
                    {
                        ...
                        if (obj.ExecuteScalar("InsUpdtGradeSubjectMaster", hashtable).Trim().ToUpper() == "S")
                            ++num2;
                        else
                            ++num1;
                    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        string str = obj.ExecuteScalar("DeleteGradeSubjectMaster", new Hashtable()
    {
      {
         "@SubjectId",
